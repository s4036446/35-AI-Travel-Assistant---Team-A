...
- Given the assistant has identified the operating airline for the user's flight, and the user has a single registered wheelchair/equipment profile
- When the user asks whether their wheelchair can be taken on board
- Then the assistant looks up the operating airline's mobility-aid and battery rules, and, where aircraft data is available, the aircraft's cargo/baggage door data, and states directly whether the registered equipment is allowed, rather than showing raw dimensions for the user to interpret
...
- Given the assistant has the operating airline's accessibility data (and, where available, the aircraft's data) and the user's equipment profile
- When the assistant compares the equipment against those requirements and finds a mismatch (e.g. a liquid-battery restriction)
